...
import {
    EditorManager, EditorDecorationsService, Range, Position, DeltaDecorationParams, DeltaDecoration,
...
export class DirtyDiffDecorator {
    ...
    protected async setDecorations(uri: string, newDecorations: DeltaDecoration[]) {
        ...
        const decorationIds = await this.editorDecorationsService.deltaDecorations(<DeltaDecorationParams>{ uri, oldDecorations, newDecorations });
    ...
    protected toDeltaDecoration(from: LineRange | number, options: ModelDecorationOptions): DeltaDecoration {
        ...
        return <DeltaDecoration>{ range, options };
    }
}
